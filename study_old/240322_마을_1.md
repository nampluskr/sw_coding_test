...
### [user_nam.cpp] 처음 풀이 변수 / 에러 확인 할 것
...
vector<int> houseList[4'001][4'001];

void merge(House& h1, House& h2) {
	if (h1.vIdx == h2.vIdx) return;
	if (villages[h1.vIdx].num_houses < villages[h2.vIdx].num_houses) swap(h1, h2);
	auto& v1 = villages[h1.vIdx];
	auto& v2 = villages[h2.vIdx];
	// v2 -> v1
	v1.min_ID = min(v1.min_ID, v2.min_ID);
	v1.num_houses += v2.num_houses;
	villageCnt--;
	for (const auto& hIdx : v2.houseList) {
		if (houses[hIdx].state = REMOVED) continue;
		houses[hIdx].vIdx = h1.vIdx;
		v1.houseList.push_back(hIdx);
	}
}

static int TC = 1;
/////////////////////////////////////////////////////////////////////
// L: 마을을 이루는 기준 거리(10 ≤ L ≤ 200)
// R: 
void init(int L, int R) {
	...
	::L = L;				// 마을을 이루는 기준 거리(10 ≤ L ≤ 200)
	::R = R;				// X좌표와 Y좌표의 최댓값(25 ≤ R ≤ L x 100)
	//::sq = sqrt(R + 1);		// bucket size (max buckst index = R / sq)
	::sq = L;
	houseCnt = 0;
	villageCnt = 0;
}

// mId : 집 ID(1 ≤ mId ≤ 1, 000, 000, 000)
// mX, mY : 집의 X좌표(1 ≤ mX ≤ R), 집의 Y좌표(1 ≤ mY ≤ R)
// mId 집이 속한 마을의 집 개수를 반환한다.
// 25,000
int add(int mId, int mX, int mY) {
	int hIdx1 = houseMap[mId] = houseCnt++;
	houses[hIdx1] = { mId, mX, mY, };
	villages[hIdx1] = { mId, 1, { hIdx1 } };
	villageCnt++;

	int sX = max((mX - L) / sq, 0), eX = min((mX + L) / sq, R / sq);
	int sY = max((mY - L) / sq, 0), eY = min((mY + L) / sq, R / sq);

	for (int i = sX; i <= eX; i++)
		for (int j = sY; j <= eY; j++)
			for (int hIdx2 : houseList[i][j]) {
				auto& house2 = houses[hIdx2];
				if (house2.state == REMOVED) continue;
				if (abs(mX - house2.mX) + abs(mY - house2.mY) > L) continue;
				merge(houses[hIdx1], house2);
			}
	houseList[mX / sq][mY / sq].push_back(hIdx1);
	return villages[hIdx1].num_houses;
}

// mId 집이 속해 있던 마을의 집 개수를 반환한다. mId 집이 없다면, -1을 반환한다
// 1,000
int remove(int mId) {
	auto iter = houseMap.find(mId);
	if (iter == houseMap.end()) return -1;

	auto& house = houses[iter->second];
	house.state = REMOVED;
	auto& village = villages[house.vIdx];
	village.num_houses--;

	if (village.min_ID == house.mId) {
		if (village.num_houses = 0) return -1;
		for (int hIdx : village.houseList) {
			if (houses[hIdx].state == REMOVED) continue;
			village.min_ID = min(village.min_ID, houses[hIdx].mId);
		}
	}
	return village.num_houses;
}


// mId 집이 속해 있는 마을에서 가장 작은 집 ID값을 반환한다. mId 집이 없다면, -1을 반환한다.
// 2,500
int check(int mId) {
	auto iter = houseMap.find(mId);
	if (iter == houseMap.end()) return -1;
	...
	int hIdx = iter->second;
	return villages[houses[hIdx].vIdx].min_ID;
}
