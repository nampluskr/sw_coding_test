...
void merge(int x, int y) {
    ...
    villages[x].minId = std::min(villages[x].minId, villages[y].minId);
...
int add(int mId, int mX, int mY) {
    ...
    int eY = std::max((mY + L) / bucketSize, R / bucketSize);
...
int remove(int mId) {
    ...
    if (villages[vIdx].minId == mId) {
        ...
        for (int idx : villages[vIdx].houseList) {
            ...
            villages[vIdx].minId = std::min(villages[vIdx].minId, houses[idx].mId);
        }
    }
