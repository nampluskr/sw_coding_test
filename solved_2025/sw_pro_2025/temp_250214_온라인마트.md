    ...
    if (mHow == 0) {
            ...
            for (int j = 1; j < MAX_COMPANIES; j++) {
                for (const Data& data : itemList[i][j]) {
                    ...
                    if (!items[idx].isClosed) {
                        ...
                        int realPrice = items[idx].mPrice - offsetDC[i][j];
                        if (realPrice > 0) {  // 0 이하면 이미 판매 종료됨
                            activeItems.push_back({data.mID, realPrice});
                        }
                    }
                }
            }
        ...
        for (int j = 1; j < MAX_COMPANIES; j++) {
            for (const Data& data : itemList[mCode][j]) {
                ...
                if (!items[idx].isClosed) {
                    // 실제 가격 계산 (lazy discount offset 적용)
                    int realPrice = items[idx].mPrice - offsetDC[mCode][j];
                    if (realPrice > 0) {
                        activeItems.push_back({data.mID, realPrice});
                    }
                }
        ...
        for (int i = 1; i < MAX_CATEGORIES; i++) {
            for (const Data& data : itemList[i][mCode]) {
                ...
                if (!items[idx].isClosed) {
                    // 실제 가격 계산 (lazy discount offset 적용)
                    int realPrice = items[idx].mPrice - offsetDC[i][mCode];
                    if (realPrice > 0) {
                        activeItems.push_back({data.mID, realPrice});
                    }
                }
