...
#define MAX_K	3400    // 도로의 최대 개수 (add 함수 최대 1400회 고려)
...
int getIdx(int mId) {
    ...
    return roadMap[mId] = numRoads++;
}

void add(int mId, int sCity, int eCity, int mToll) {
	int rIdx = getIdx(mId);
	roads[rIdx] = { sCity, eCity, mToll, false };  // isRemoved 초기값 false
	graph[sCity].push_back({ eCity, mToll, rIdx, 0 });
}
...
void remove(int mId) {
    // 존재하지 않는 mId에 대한 예외 처리
    if (roadMap.find(mId) != roadMap.end()) {
        int rIdx = roadMap[mId];
        roads[rIdx].isRemoved = true;  // 플래그만 변경
    }
}
...
int cost(int M, int sCity, int eCity) {
    ...
    while (!pq.empty()) {
        ...
        for (const Edge& edge : graph[curr.to]) {
            // 삭제된 도로는 건너뛰기
            ...
            if (newCost < minCost[nextCity][curr.usedCoupons]) {
                ...
                pq.push({ nextCity, newCost, 0, curr.usedCoupons });
            }
            ...
            if (curr.usedCoupons < M) {
                ...
                if (discountedCost < minCost[nextCity][curr.usedCoupons + 1]) {
                    ...
                    pq.push({ nextCity, discountedCost, 0, curr.usedCoupons + 1 });
                }
            }
        }
    }
